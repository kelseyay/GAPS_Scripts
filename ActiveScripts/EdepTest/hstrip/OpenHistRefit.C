...
TFile *h = TFile::Open("h_l0r0m0s1.root");
...
TH1F * hstrip = new TH1F ("h_l0r0m0s1","h_l0r0m0s1", NBins, xlow,xhigh);
hstrip = (TH1F*)h->Get("h0_l0r0m0s1");
...
hstrip->Fit(f_landau_mod,"R");
`
hstrip->Draw();

cout << "Fit Parameter 1 " << 
...
hstrip->SetLineColor(1);

hstrip->GetXaxis()->SetRangeUser(xlow, xhigh);
hstrip->GetXaxis()->SetTitle("Energy Deposition of Hit #times  Cos(#theta) [MeV]");
hstrip->GetYaxis()->SetTitle("Number of Hits on Track");
