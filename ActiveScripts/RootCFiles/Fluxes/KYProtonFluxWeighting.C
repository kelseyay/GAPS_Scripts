...
void ExampleWeighting(){
    ...
    for(unsigned int i = 0; i < TreeRec->GetEntries(); i+=MainLoopScaleFactor)
    {
	    ...
	    if(AngularRegion < 0) continue;
            double RateScale = FluxScaleFactor*AcceptanceScale*GProtonTotalFluxUnscaled.at(AngularRegion)->Eval(Event->GetPrimaryBetaGenerated());
    ...
    char text[400];
    sprintf(text, "%sweightedbetaProtonv300.png", Directory.c_str());
    ...
    sprintf(text, "%sweightedcosthetaProtonv300.png", Directory.c_str());
