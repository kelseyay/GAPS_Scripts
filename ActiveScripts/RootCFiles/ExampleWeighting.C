...
void ExampleWeighting(){
    ...
    char why[400];
    sprintf(why,"%s/%s.root","/data1/nextcloud/cra_data/nobackup/MC/v3.0.0/full/mu-/", "mu-_gaps_triggerlevel1_FTFP_BERT_1757442*");
    ...
    TH1D * HBeta = Plotting.DefineTH1D("HBeta", xbin, 0, xlim, "beta", "Counts", 0.5, 1e6);
    ...
    //else HPrimaryBeta = new TH1D("HPrimaryBeta", "", BetaBins, PrimaryBetaLowHigh.at(0), PrimaryBetaLowHigh.at(1));
    ...
    for(unsigned int i = 0; i < TreeRec->GetEntries(); i+=MainLoopScaleFactor)
    {
            ...
            //choose a reconstruction for beta
    ...
    //HBeta->GetYaxis()->SetRangeUser(0, 100000);

    TLegend* LegBeta = new TLegend(0.65, 0.8, 0.95, 0.95);
    LegBeta->SetFillColor(0);

    HBeta->SetLineColor(6);
    HBeta->Draw("hist");
    LegBeta->AddEntry(HBeta, "weighted beta rate", "l");
    HPrimaryBeta->SetLineColor(1);
    HPrimaryBeta->Draw("histsame");
    LegBeta->AddEntry(HPrimaryBeta, "unweighted beta", "l");
    ...
    cout<<"muon rate: "<<HBeta->Integral(0,100)<<"Hz"<<endl;
