// plot_pedestal_comparison.C
{
    ...
    // Create histogram (same definition as original code) //Score from module basis
    TH2D *hdiff = new TH2D("hdiff", "Score from Threshold Recovery",
                           nrows, 0, nrows,
                           nlayers*nmods, 0, nlayers*nmods);

    // Create histogram (same definition as original code) //Instead pulsing 2 channels per module but make decision to pulse module
    ...
    double before_data[7][6][6][32] = {0}; //Keep track of all sigmas before CMN subtraction
    double after_data[7][6][6][32] = {0}; //Keep track of all sigmas after CMN subtraction
    int pulsed_ch[7][6][6][ncn] = {0}; //NCN pulsed channels
    double mag_improv[7][6][6] = {0}; //Each module has an associated magnitude of improvement of total sigma of all strips
    int score_improv[7][6][6] = {0}; //Score improvement depends on the threshold
    ...
    std::ofstream myfile;
    myfile.open("output/FullInfoMod.txt");
    myfile << "User Set Threshold = " << xthresh << endl;
    myfile << "Layer\tRow\tMod\tStrip\tPre-CMN\tPost-CMN" << endl;

    const int xmax = nstrips - 2; //xmax is the maximum score a module can get
    int xtally[xmax] = {0}; //Keeping track of score
    ...
    for (int l = 0; l < nlayers; l++) {
        for (int r = 0; r < nrows; r++) {
            for (int m = 0; m < nmods; m++) { //Going over all modules
                ...
                double sum_sigma = 0;
                int xscore = 0;
                for(int ch = 0; ch < nstrips; ch++ ){ //Go over all of the strips
                ...
                mag_improv[l][r][m] = sum_sigma;
                score_improv[l][r][m] = xscore;
                xtally[xscore]++;
                myfile << "Pulsed channel = " << pulsed_ch[l][r][m][0] << "and" << pulsed_ch[l][r][m][1] << endl;
                myfile << "Score = " << xscore << endl;
                myfile << "Total sigma improvement of mod " << m << " = " <<  sum_sigma << endl << endl;
    ...
    //Look at each module. Use score and magnitude to determine which channels to pulse.
    //Start by just making sure you can organize by score and magnitude
    //cout << "Max score is " << xmax << endl;

    std::ofstream rankfile;
    rankfile.open("output/RankedMods.txt");
    rankfile << "User Set Threshold = " << xthresh << endl;
    rankfile << "Layer\tRow\tMod\tStrip1\tStrip2\tScore\tMag Improv" << endl;
        ...
        for (int l = 0; l < nlayers; l++) {
            for (int r = 0; r < nrows; r++) {
                for (int m = 0; m < nmods; m++) { //Going over all modules
                        ...
                        if(mag_improv[l][r][m] < -10) continue; //Don't pulse a module if it makes it worse

                        if(score_improv[l][r][m] == xmax - x){
                        ...
                        for(double val : mag_comp){ //Iterate through the mag vector, starts with no elements.
                            ...
                            if(mag_improv[l][r][m] < val) tkr++;
                        }

                        auto itloc = mag_comp.begin() + tkr;
                        auto itloc_txt = text.begin() + tkr;
                        mag_comp.insert(itloc,mag_improv[l][r][m]);
                        words = TString::Format("%d\t%d\t%d\t%d\t%d\t%d\t%f\n",l,r,m,pulsed_ch[l][r][m][0],pulsed_ch[l][r][m][1],score_improv[l][r][m], mag_improv[l][r][m]);
    ...
    for (int i = 0; i <= nrows; i++) {
        lines.push_back(new TLine(i, 0, i, nlayers * nmods));
    }
    for (int i = 0; i <= nlayers; i++) {
        lines.push_back(new TLine(0, i * nmods, nrows, i * nmods));
    }
    ...
    hdiff->SetMinimum(0);
    hdiff->SetMaximum(nstrips-ncn);  // Maximum value 30
