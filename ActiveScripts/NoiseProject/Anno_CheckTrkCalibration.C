...
void EvaluatePedestals(TString flist, int cnmod ,Long64_t nev,Long64_t evidmin, Long64_t evidmax, TString ddir,TString suffix){
  ...
  double  sig0[NLAYERS][NROWS][NMODULES][NCHANNELS];hsmip
  ...
  for (uint it=1 ; it<=niter; it++){ // iterate
    ...
    ////////////////////////////////////////////////////////
    int cnt=0;
    for (uint ie=0; ie<craw->GetEntries(); ie++){ //(Closed around line 839)
	...
	double cncn = 0;
	if(
	   !disconnected[layer][row][module][channel] && //Disconnected taken into account. Probably relevant to the masking
	   true){
	  cncn = cn[layer][row][module][cnindex]*gain[layer][row][module][channel]  ;
	}
	///////////////////////////////////////////
	adc[layer][row][module][channel] = val-cncn; //Okay I get it, you're rewriting adc to have the value - cncn for each channel. If the channel is disconnected this = 0.
	///////////////////////////////////////////

	if(it==niter){ //@last iteration

	  if( module_ok[layer][row][module][half] ){ //Half is hard coded?!

            hsped[layer][row][module]->Fill(adc[layer][row][module][channel]-ped[layer][row][module][channel]);
	    hsok[irank]->Fill(adc[layer][row][module][channel]-ped[layer][row][module][channel]); //What is hsokay[irank]?
	    if(!hs[layer][row][module][channel]) hs[layer][row][module][channel] = new TH1F(Form("hs_%i_%i",100*layer+10*row+module,channel),"R-PED-B",100,-50,50);
				//If not hs[l][r][m][ch] then make a new hs histogram R - PED - B (which would leave g_i * cmn)

	    hs[layer][row][module][channel]->Fill(adc[layer][row][module][channel]-ped[layer][row][module][channel] );
					//This quantity is not -B unless that hpapens earlier? In any case, hs is filled at a channel level. adc has - cncn actually!
	  }else{

            hsmip[layer][row][module]->Fill(adc[layer][row][module][channel]-ped[layer][row][module][channel]);
                //Else in module_okay, so module_okay must be determining whether or not the MIP was included.
	    hsnotok[irank]->Fill(adc[layer][row][module][channel]-ped[layer][row][module][channel]);
				//hsnotok[irank] is filled with the same thing.
        ...
        if(it==niter_cn-1){ //@ last iteration before cn correction
          ...
          //hsall does the adc - ped before the cn correction. So that must by where -B is implemented?
